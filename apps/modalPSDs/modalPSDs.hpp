...
int modalPSDs::allocate(const dev::shmimT& dummy)
{
   ...
   clock_gettime(CLOCK_ISIO, &m_freqStream->md->writetime);
...
void modalPSDs::psdThreadExec()
{
   ...
   while (shutdown() == 0)
   {
      ...
      while (m_psdRestarting == false && !shutdown())
      {
         ...
         //Set the time of last write
         clock_gettime(CLOCK_ISIO, &m_rawpsdStream->md->writetime);
         ...
         //Set the time of last write
         clock_gettime(CLOCK_ISIO, &m_avgpsdStream->md->writetime);
