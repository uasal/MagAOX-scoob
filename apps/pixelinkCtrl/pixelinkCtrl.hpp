...
/** MagAO-X application to control a Pixelink camera
  ...
  */
class pixelinkCtrl : public MagAOXApp<>, public dev::stdCamera<pixelinkCtrl>, public dev::frameGrabber<pixelinkCtrl>, public dev::dssShutter<pixelinkCtrl>, public dev::telemeter<pixelinkCtrl>
{
...
public:
   ...
   static constexpr bool c_stdCamera_vShiftSpeed = truefalse; ///< app:dev config to tell stdCamera to expose vertical shift speed control
...
protected:
   ...
   pixelinkHandle m_cameraHandle {0};
...
int pixelinkCtrl::appLogic()
{
   ...
   if(dev::dssShutter<pixelinkCtrl>::appLogic() < 0)
   {
      return log<software_error, -1>({__FILE__, __LINE__});
   }
...
int pixelinkCtrl::onPowerOff()
{
   ...
   if(dssShutter<pixelinkCtrl>::onPowerOff() < 0)
   {
      log<software_error>({__FILE__, __LINE__});
   }
...
inline 
int pixelinkCtrl::setTempControl()
{
   //Always on
   m_tempControlStatus = true;
...
inline 
int pixelinkCtrl::setVShiftSpeed()
{
   m_reconfig = true;
   recordCamera(true);
   return 0;
...
inline
int pixelinkCtrl::capExpTime(piflt& exptime)
{
...
inline 
int pixelinkCtrl::setShutter( int sh )
{
   return dssShutter<pixelinkCtrl>::setShutter(sh);
}
...
{
   if( frameGrabber<pixelinkCtrl>::loadImageIntoStreamCopy(dest, m_available.initial_readout, m_width, m_height, m_typeSize) == nullptr) return -1;
