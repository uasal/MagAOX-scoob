                                      ...
                                      const file::stdFileName<verboseT> &lastFile )
{
    ...
    timespecX prevts = firstFile.timestamp();
    prevts.time_s -= 60; // Move 60 seconds in future.  This is a config setting

    timespecX follts = lastFile.timestamp();
    ...
    file::stdSubDir subdir = firstFile.subDir();
    int             ndays  = 0;
    ...
    while( prevLogFound == false && ndays < 5 ) // 5 is a config setting
    {
        ...
        for( size_t n = tmp_flist.size() - 1; n != static_cast<size_t>( -1 ); --n )
        {
            ...
            {
                continue; // on any errors we assume it's not a valid log file and just go around
            }
    ...
    if( prevLogSubDir == follLogSubDir ) // special case, probably most common
    {
        ...
        else
        {
            ...
            {
                throw xwcException( "miscounted the number of files somewhere" );
            }
        }

        for( size_t n = prevLogFile_n; n < follLogFile_n; ++n )
        {
            try
            {
            ...
            {
                std::throw_with_nested( xwcException( "error adding file to map" ) );
            }
        }
    }
    else
    {
        ...
        for( size_t n = 0; n < follLogFile_n; ++n )
        {
            try
            {
                m_appToFileMap[dev].insert( file::stdFileName<verboseT>( tmp_flist[n] ) );
...
int logMap::getNextLog( char *&logAfter, char *logCurrent, const std::string &appName )
{
...
int logMap::loadFiles( const std::string &appName, const flatlogs::timespecX &startTime )
{
    if( m_appToFileMap[appName].size() == 0 )
    {
        std::cerr << "*************************************\n\n";
