...
struct logInMemory
{
    ...
    int loadFile( file::stdFileName const &lfn );
};
...
struct logMap
{
    ...
    typedef std::map<std::string, std::set<file::stdFileName, file::compStdFileName>> appToFileMapT;
    ...
    mx::error_t loadAppToFileMap( const std::string &dir,             /**< [in] the directory to search for files
                           ...
                           const file::stdFileName &firstFile, ///< [in] the first file that needs coverage
                           const file::stdFileName &lastFile   ///< [in] the last file that needs coverage
    );
